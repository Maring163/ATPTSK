...
class FirstTSK(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_fuzzy_set, mf='Gaussian', frb_type='CoCo-FRB', tnorm='prod'):
        ...
        self.spread = nn.Parameter(
            torch.full([num_fuzzy_set, in_dim], 2 ** 0.5,dtype=torch.float64))  # [num_fuzzy_set, in_dim]
        ...
        # gate parameters initialization in ATPTSK
        if self.mf == 'GCGMF':
            # the gate values, m(lambda), is initialized to 1.0
            self.lambdas = nn.Parameter(
                torch.full([1, in_dim], math.sqrt(-0.1 * math.log(1 - 0.1))))  # [num_fuzzy_set, in_dim]
    ...
    def forward(self, model_input):
        ...
        # calculate membership values [num_sam, num_rule, num_fea]
        if self.mf == 'Gaussian':
            membership_value = gauss(model_input.unsqueeze(1), self.center, self.spread)
        elif self.mf == 'GCGMF':
            membership_value = generalized_cgmf(model_input.unsqueeze(1), self.center, self.spread, self.lambdas)
        else:
        ...
        if self.tnorm == 'prod':  # Product T-norm
            fir_str = membership_value[:, fs_ind, range(in_dim)].prod(dim=2)
        elif self.tnorm == 'min':  # Minimum T-norm
            fir_str = membership_value[:, fs_ind, range(in_dim)].min(dim=2).values
        elif self.tnorm == 'atp-softmin': # ATP-softmin
            fir_str = atp_softmin(membership_value[:, fs_ind, range(in_dim)], dim=2)
        else:
        ...
        # check system crash
        if torch.isnan(fir_str).any():
            print('\033[31m' + 'Nan occurs during optimization, the system crashes!' + '\033[0m')

        # calculate rule outputs, [num_sam,num_rule,out_dim]
        rule_output = (self.con_param[:, :, 1:] @ model_input.T).T + self.con_param[:, :, 0].T # [num_sample, num_fuzzy, num_class]

        # de-fuzzy for computing the model outputs
        fir_str_bar = fir_str / fir_str.sum(dim=1).unsqueeze(1)
